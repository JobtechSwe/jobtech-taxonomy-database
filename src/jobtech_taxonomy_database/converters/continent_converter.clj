...
(defn find-id
  ...
  [id]
  (let [nano (get-in (open-json) [:continent (keyword (str id)) :conceptId])] ;TODO in other files: change "country" to relevant keyword
    (if nano nano (generate-new-id))))

(defn converter
  "Immutable language converter."
  [data]
  (let [nano-id (find-id  (:continentid  data))] ;TODO in other files: change "countryid" to relevant keyword
  [{:concept/id nano-id
