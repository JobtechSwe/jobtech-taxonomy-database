...
(defn convert-new-skill [{:keys [id-68 term-68 skill-headline]}]
  ...
  (if (not (u/get-concept-by-legacy-id id-68 t/skill ))
    (let [ concept (u/create-concept t/skill term-68 term-68 id-68)]
      [
       concept
       (u/create-term-from-concept concept)
       (u/create-broader-relation-to-concept concept (u/get-entity-if-exists-or-temp-id skill-headline :skill-headline))
       ]
      )
    )
  )
...
(defn convert-updated-skill [{:keys [skillid term]}]
  (let [entity-id (u/get-concept-by-legacy-id skillid t/skill) ;; borde heta get-entity-id-by-legacy-i
...
(defn convert-replaced-skill [{:keys [skillid term skillidref]}]
  (let [entity-id-old-skill (u/get-concept-by-legacy-id skillid t/skill)
        entity-id-new-skill (u/get-concept-by-legacy-id skillidref t/skill)
        ]
